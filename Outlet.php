...
// 1. mysqli_query
$sql=$this->mysqli->query("SELECT * FROM outlet") or die($this->CekError());
...
public function detail($kodeOutlet){
// 1. mysqli_query
$sql=$this->mysqli->query("SELECT * FROM outlet WHERE kode_outlet='".$kodeOutlet."'") or die ($this->CekError());
$dataOutlet=$sql->fetch_object();
// 2. jika datanya ada
if(isset($dataOutlet)){
...
function update($kodeOutlet,$data){
...
$this->mysqli->query("UPDATE outlet SET ".$script_update." WHERE kode_outlet='".$kodeOutlet."'") or die ($this->CekError());
// 4. tutup koneksi
$this->TutupKoneksi();
}
function hapus($kodeOutlet){
// 1. Jalankan perintah delete query
$this->mysqli->query("DELETE FROM outlet WHERE kode_outlet='$kodeOutlet'");
// 2. tutup koneksi
$this->TutupKoneksi();
}
...
$sql_simpan="INSERT INTO outlet (".$kolom_nya_baru.") VALUES (".$nilai_nya_baru.")";
